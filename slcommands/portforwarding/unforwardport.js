...
export async function execute(client, interaction) {
    ...
    let da = interaction.options.getString('da');
    ...
    await exec(`/usr/sbin/iptables -L -t nat | grep "dpt:${port}"`, async(err, stdout, stderr) => {
        stdout = stdout.trim();
        if(stdout !== "") {
            await exec(`/usr/sbin/iptables -t nat -D PREROUTING -p tcp --dport ${port} -j DNAT --to-destination ${da}`, async(err, stdout, stderr) => {
                stdout = stdout.trim();
                if(stdout == '') {
